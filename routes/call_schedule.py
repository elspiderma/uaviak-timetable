...
async def call_schedule(msg: Message):
    ...
    server, photo, hash_ = await upload_photo(upload_server, os.path.join(config.STATIC_DIR, 'schedule.jpg'))

    loaded_photo = await msg.api.photos.save_messages_photo(photo, server, hash_)
